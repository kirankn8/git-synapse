...
def test_abandoned_runs_are_reconciled_and_stop_blocking(db):
    """A killed run must not block ingestion forever.

    ``POST /api/ingest/refresh`` refuses to start while a run is in flight. A
    container killed mid-run leaves its row stuck in ``running`` with nothing
    left to update it, so without reconciliation one bad shutdown would disable
    ingestion permanently.
    """
    from git_synapse.ingest.pipeline import active_run, reconcile_stale_runs

    with connection() as conn:
        stale_id = conn.execute(
            """
            INSERT INTO ingest_run (kind, trigger, status, started_at)
            VALUES ('sync', 'test', 'running', now() - interval '24 hours')
            RETURNING id
            """
        ).fetchone()[0]

...
The table is the test. Every row here is a string somebody could plausibly have
in their clipboard, and getting one wrong is not a parse error the user sees --
it is a repository quietly filed under the wrong owner, or an organisation of
eight thousand enumerated when they asked for one repository.
"""
...
@pytest.mark.parametrize("raw,provider,owner,repo,api", [
    ...
    # A host nobody has written a client for. Not an error: cloning is all the
    # coupling maths needs.
...
def test_a_gitlab_group_may_nest():
    """`gitlab-org/security/gitlab` is one project in a subgroup, not a project
    called `security` -- and the owner is everything above the last segment."""
...
def test_the_clone_url_is_always_https():
    """The mirror is read-only and anonymous unless a token is configured; an
    ssh remote would need a key in the container nothing else here wants."""
